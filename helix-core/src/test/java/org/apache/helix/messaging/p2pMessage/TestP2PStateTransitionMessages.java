...
public class TestP2PStateTransitionMessages extends BaseStageTest {
  ...
  public void testAvoidDuplicatedMessageWithP2PEnabled() throws Exception {
    ...
    // Now, the old master finish state transition, and the second master also finishes state
    // transition.
    // Then the preference list has changed, so now the new master is different from previously
    // calculated new master
    // but controller should not send S->M to newly calculated master, but should send M-S to
    // second master.
