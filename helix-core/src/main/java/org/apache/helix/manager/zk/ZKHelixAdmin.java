...
public class ZKHelixAdmin implements HelixAdmin {
  ...
  @Override
  public void purgeOfflineInstances(String clusterName, Map<String, Long> customizedPurgeMap) {
  ...
  private Map<String, InstanceConfig> findTimeoutOfflineInstances(String clusterName,
      Map<String, Long> customizedPurgeMap) {
    Long timeout = customizedPurgeMap.get(OFFLINE_NODE_PURGE_TIMEOUT);
    if (timeout == null) {
    ...
    for (String instanceName: instanceConfigMap.keySet()) {
      ...
      if (lastOfflineTime == -1 || timeout == null ||
          System.currentTimeMillis() - lastOfflineTime < timeout) {
        instanceConfigMap.remove(instanceName);
      }
    }
